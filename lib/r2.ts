...
async function fetchManifest(): Promise<R2Manifest> {
  if (typeof window !== "undefined") {
    // Fallback for client-side: load from a hosted copy
  ...
  // Server-side: read from the project root (generated by scripts/generate_r2_manifest.js)
  const fs = await import("fs/promises");
  const path = await import("path");
  const raw = await fs.readFile(
    path.join(process.cwd(), "r2-manifest.json"),
    "utf8"
  );
